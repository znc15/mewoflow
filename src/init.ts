...
function agentsTemplate(): string {
  return [
    ...
    "- For a pending task, first run `mewoflow propose-task --title \"...\" --slug \"descriptive-kebab-slug\" --session <id>`, then ask the user whether to create the task. If Claude interprets the user as confirming, run `mewoflow confirm-task --session <id>`; if cancelling, run `mewoflow cancel-task --session <id>`.",
    ...
    "- Show the plan to the user before implementation. If Claude interprets the latest user response as approving the plan, run `mewoflow approve-plan --prompt \"<user approval>\" --session <id>` before `mewoflow check plan`; do not rely on hardcoded approval phrases.",
...
function claudeTemplate(): string {
  return [
    ...
    "- Pending task ids are not final. Use `mewoflow propose-task --title \"...\" --slug \"descriptive-kebab-slug\" --session <id>` before asking whether to create the task. Then run `mewoflow confirm-task --session <id>` or `mewoflow cancel-task --session <id>` based on Claude's interpretation of the user's response.",
    ...
    "- Plan must include a fresh shortcut/existing-solution scan and be shown to the user before implementation. When Claude interprets the user as approving it, run `mewoflow approve-plan --prompt \"...\" --session <id>` before `mewoflow check plan`.",
