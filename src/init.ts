...
function agentsTemplate(): string {
  return [
    ...
    "- During `implement`, PreToolUse blocks frontend/backend file edits until a relevant local skill is discovered, read, or invoked. Browse `.claude/skills/` and `~/.claude/skills/`; hooks match skills by name/description keywords, not fixed built-in skill names.",
...
function claudeTemplate(): string {
  return [
    ...
    "- Before frontend/backend implementation edits, discover and read or invoke a relevant local skill from `.claude/skills/` or `~/.claude/skills/`; hooks enforce this at the implement gate.",
...
function rulesTemplate(): string {
  return [
    ...
    "- Discover and apply relevant skills during plan, implement, and review; browse `.claude/skills/` and user/global skills instead of hardcoding per-project skill names.",
    "- At `implement`, hooks require a traced read/invocation of a relevant discovered local skill before editing frontend/backend files. Optional `.mewoflow/domain-skills.json` can tune path heuristics only.",
