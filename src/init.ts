...
function agentsTemplate(): string {
  return [
    ...
    "",
    "## Agent Teams (multi-agent development)",
    "",
    "Use Agent Teams for parallel work during `implement`, `verify`, and `review` — not during research, grill, or plan. The team lead owns the MewoFlow session and gate transitions.",
    "",
    "### When to spawn teammates",
    "",
    "- After `approve-plan` and `mewoflow check plan`, the lead enters `implement` and may spawn teammates from slices in `plan.md`.",
    "- During `implement`, assign each teammate exclusive paths (directories or files) before parallel edits.",
    "- During `verify` / `review`, optional teammates may run tests or review assigned files; the lead writes evidence and advances gates.",
    "",
    "### Role patterns",
    "",
    "- **Lead**: Runs `mewoflow check`, `approve-plan`, `rework`, `approve-archive`; merges teammate output; resolves shared files (exports, routing, config).",
    "- **Explore teammate**: Read-only search and codebase exploration to unblock implementers; no implementation writes or gate commands.",
    "- **Implement teammate**: Edits only assigned paths; reports changed files and summary to the lead when idle.",
    "- **Review teammate**: Reviews assigned files and reports findings to the lead; does not edit code during the `review` gate.",
    "",
    "### File ownership",
    "",
    "- One owner per file; never let two teammates edit the same path.",
    "- Teammates avoid `.mewoflow/runtime/` and `task.json`; evidence markdown is usually lead-owned unless the lead assigns a fragment.",
    "- Read `.mewoflow/rules.md` and active task `research.md`, `grill.md`, `plan.md` before first implementation edit (PreToolUse enforces this).",
    "",
    "### MewoFlow commands",
    "",
    "- **Lead only**: `mewoflow check <gate>`, `approve-plan`, `rework`, `approve-archive`, `split-task`.",
    "- **Teammates**: `mewoflow status` to read state; do not advance gates or approve plans.",
    "",
    ...
    "4. For each gate, write the required evidence file in `.mewoflow/tasks/<task-id>/`, record a Model evidence sufficiency judgment with reason, then advance with `mewoflow check <gate> --session <id>`. Do not skip from research/grill/plan into implementation.",
    "5. Before implementation, show the plan, record explicit approval with `mewoflow approve-plan --prompt \"...\" --session <id>`, then run `mewoflow check plan --session <id>`, read `.mewoflow/rules.md`, `research.md`, `grill.md`, and `plan.md`, and record Model domain judgment for intended write targets (frontend/backend/none) with reasons.",
    ...
    "- During `implement`, PreToolUse blocks frontend/backend file edits until a relevant local skill is discovered, read, or invoked. Before those edits, record a Model domain judgment for each intended write target (frontend/backend/none) with reason. Browse `.claude/skills/` and `~/.claude/skills/`; hooks match skills by name/description keywords, not fixed built-in skill names.",
    ...
    "- Record why continuing to ask is now low-value before leaving `grill`.",
    "- Before finalizing `plan.md`, run a fresh WebSearch/WebFetch/MCP/skill shortcut scan and record MVP slice, phases, deferred work, risks, and verification; LLM decides structure and sections.",
    "- For from-scratch epic projects, keep the first task as the parent epic, list child tasks in plan, then split them with `mewoflow split-task --from-plan` after plan approval.",
    ...
    "- Use `mewoflow check <gate>` to advance a gate only after the evidence file is complete and includes a Model evidence sufficiency judgment with reason.",
...
function claudeTemplate(): string {
  return [
    ...
    "Optional: If you enable Claude Code Agent Teams, MewoFlow also wires the Agent Teams hook events (TeammateIdle / TaskCreated / TaskCompleted) for lightweight coordination hints. See AGENTS.md § Agent Teams for lead/teammate roles, file ownership, and when to spawn during implement/verify/review.",
    "",
    ...
    "2. On new development work, report the simple / standard / epic judgment before doing anything else. Prefer AskQuestion for judgment review, but still run `mewoflow accept-judgment --classification <simple|standard|epic>` or `mewoflow reject-judgment` after interpreting the response.",
    ...
    "4. Work one gate at a time: fill the evidence file, record a Model evidence sufficiency judgment with reason, run `mewoflow check <gate> --session <id>`, and follow the next gate reported by the CLI/hook notice.",
    "5. Before editing implementation files, show the plan, record approval with `mewoflow approve-plan --prompt \"...\" --session <id>`, run `mewoflow check plan --session <id>`, read `.mewoflow/rules.md` plus the active task research/grill/plan files, and record Model domain judgment for intended write targets (frontend/backend/none) with reasons.",
    ...
    "- Before frontend/backend implementation edits, record Model domain judgment for each intended write target (frontend/backend/none) with reason, then discover and read or invoke a relevant local skill from `.claude/skills/` or `~/.claude/skills/`; hooks enforce this at the implement gate.",
...
function rulesTemplate(): string {
  return [
    ...
    "- Before each `mewoflow check <gate>`, record a Model evidence sufficiency judgment with reason in the active gate evidence file; if evidence is insufficient, continue working instead of advancing.",
    "- Discover and apply relevant skills during plan, implement, and review; browse `.claude/skills/` and user/global skills instead of hardcoding per-project skill names.",
    "- At `implement`, record Model domain judgment for intended write targets (frontend/backend/none) with reasons; hooks require a traced read/invocation of a relevant discovered local skill before editing frontend/backend files. Optional `.mewoflow/domain-skills.json` can tune path heuristics only.",
